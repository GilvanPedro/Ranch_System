...
public class FuncionarioController {
    ...
    // EDITAR FUNCIONARIO
    public String editarFuncionario(String cpfAntigo, String nome, String cpfNovo, double salario, String telefone) {
        lerArquivo();
        
        if (nome == null || nome.trim().isEmpty()) return "O nome não pode estar vazio.";
        if (!validarCpf.cpfValido(cpfNovo)) return "CPF inválido.";
        if (!validarTelefone.telefoneValido(telefone)) return "Telefone inválido.";
        if (salario <= 0) return "O salário deve ser maior que zero.";
        
        if (!cpfAntigo.equals(cpfNovo) && cpfExiste(cpfNovo)) return "Novo CPF já cadastrado.";

        for (int i = 0; i < funcionariosLista.size(); i++) {
            if (funcionariosLista.get(i).getCpf().equals(cpfAntigo)) {
                Funcionario f = new Funcionario(nome, cpfNovo, salario, telefone.replaceAll("\\D", ""));
                funcionariosLista.set(i, f);
                escreverArquivo();
                return "Sucesso";
            }
        }
